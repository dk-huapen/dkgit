		...
		function mUp(count_id,kks){//鼠标右键点击响应
			...
			//alert(count_id);
		    	window.open("http://dklovelich.iok.la/taizhang/equipment_account/look_content.php?id="+count_id)
...
str = "http://dklovelich.iok.la/taizhang/equipment_account/look_content.php?id=" + id;
  			if (window.XMLHttpRequest) {
    				// code for IE7+, Firefox, Chrome, Opera, Safari
    				xmlhttp=new XMLHttpRequest();
  			} else { // code for IE6, IE5
  			...
  			xmlhttp.open("GET","lib/QRPageri.php?id="+str,true);
  			xmlhttp.send();
		}
function plot(){
//查询曲线函数
