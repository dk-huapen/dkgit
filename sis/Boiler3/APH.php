<html>
	...
	<body>
		<?php include("top.php")?>
		<center><h1>3号炉空预器本体系统</h1></center>
		<script>
			var page = 111;
			var test =<?php
			include('../conn.php');
			$sql="SELECT kks,name,value,unit,updatetime,HH,H,HHH,L,LL,LLL,flag,angle,indexID,X,Y FROM sis where page=111 union SELECT kks,name,value,unit,updatetime,HH,H,HHH,L,LL,LLL,flag,angle1,indexID,X1,Y1 FROM sis where page1=111";
	...
	<!--SIS画面-->
		<svg width="1860" height="1000" viewBox="0 0 1860 1000" fill="gray">
			...
			<?php
				//包含磨煤机模板
				include("../TemplateSvg/APHTemplate.php");
			?>
			<!--包含具体名称的元素-->
