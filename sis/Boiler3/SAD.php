<html>
	<head>
    		<title>3号炉二次风挡板</title>
		<?php include("../header.php")?>
	</head>
	<body>
		<?php include("top.php")?>
		<center><h1>3号炉二次风挡板</h1></center>
		<script>
			var page = 114;
			var test =<?php
			include('../conn.php');
			$sql="SELECT kks,name,value,unit,updatetime,HH,H,HHH,L,LL,LLL,flag,angle,indexID,X,Y FROM sis where page=114 union SELECT kks,name,value,unit,updatetime,HH,H,HHH,L,LL,LLL,flag,angle1,indexID,X1,Y1 FROM sis where page1=114";
	...
	<!--SIS画面-->
		<svg width="1860" height="1000" viewBox="0 0 1860 1000" fill="gray">
			...
			<?php
				//包含二次风门模板
				include("../TemplateSvg/SADTemplate.php");
			?>
			<!--包含具体名称的元素-->
